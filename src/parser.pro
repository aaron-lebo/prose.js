...
symbol = {id, prefix, infix, power: 0,
    symbols[id] | (
        symbols[id] := ( 
            id: id,
            prefix: prefix | t {nil},
...
infix = {ids, power, right,
    if(ids Array?, ids, ids Array) map(i,  
        i symbol(
            nil, 
            {l, t, ts,
                node(i, l, expression(ts, right ?(power - 1, power)), t.line)
            },
            power 
        ) 
    ) 
...
expression = {tokens, power: 0,
    token = tokens.shift()
    sym = token.type symbol
    left = sym.prefix(token, tokens) 
    for(tokens[0] & power < symbol(tokens[0].type).power,
        token := tokens.shift()
        sym := token.type symbol 
